...
clock.start("  setting project and reading templates");
...
const  FS                 = require("fs")
      ...
      ;
clock.now("  setting project and reading templates");
...
clock.start("  read article files to memory");
ARTICLES = FS.readdirSync(FOLDER_ROOT, {encoding:"utf8"})
          ...
          ;
clock.now("  read article files to memory");
...
clock.start("  re-arrange articles by their creation-date (birth-date)");
...
clock.now( "  re-arrange articles by their creation-date (birth-date)");
...
clock.start("  calculate most-recent article");
latest_article = ARTICLES[ ARTICLES.length - 1 ];
clock.now(  "  calculate most-recent article");
...
clock.start("  apply correct-dates to back to folders");
...
clock.now("  apply correct-dates to back to folders");
...
clock.start("  fixing content - Unicode/line-feed");
...
clock.now("  fixing content - Unicode/line-feed");
...
clock.start("  optionally generate a limited-length-content by text-markers");
...
clock.now("  optionally generate a limited-length-content by text-markers");
...
clock.start("  generate keywords based on article content and title, and constants");
ARTICLES.forEach(function(article){
...
clock.now("  generate keywords based on article content and title, and constants");
...
clock.start("  add for each article an array of related articles");
...
clock.now("  add for each article an array of related articles");
...
clock.start("  generate article README.md (simplified index.html ...) and index.html (with head)");
ARTICLES.forEach(function(article, index){
  var TEMPLATE_RELATED = '<a href="###HREF###" title="###RELATED_KEYWORDS###">###TITLE###</a>'
     ,matchings
     ,html
     ;

  NATURAL_COMPARE.modifier = function(a){return a.post_title;}

  //the array will surely include 10 articles (the '%' - modulu is used in case this is one of the last articles, so the "next" articles will be the taken from the top of the list - the first ones).
  matchings = article.matchings
              .slice(0,10)
              .sort(NATURAL_COMPARE)
              .map(function(matched_article){
                 return TEMPLATE_RELATED
                          .replace("###HREF###"             , "https://icompile.eladkarako.com/" + matched_article.post_name  )
                          .replace("###RELATED_KEYWORDS###" , (matched_article.keywords_matched || []).join(", ")                             )
                          .replace("###TITLE###"            , matched_article.post_title                                      )
                        ;
               })
               .join("<br/>\r\n    ") + "<br/>\r\n"
              ;
...
clock.now("  generate article README.md (simplified index.html ...) and index.html (with head)");
...
clock.start("  write the README.md for the whole project, its just an index with titles and dates (no content)");
...
clock.now("  write the README.md for the whole project, its just an index with titles and dates (no content)");
...
clock.start("  write sitemap.xml for web-crawlers and search-engines");
var sitemap = [].concat(
...
clock.now("  write sitemap.xml for web-crawlers and search-engines");
